...
class GPUNormRunner(Runner):
    ...
    def _prepare_input(self, batch):
        ...
        # Squeeze first dimension in case we use FullImageMaskDataset that every full image add one dimension
        if batch['image'].dim() == 5:
            for k, v in batch.items():
                # If its a tensor delete first dimension if not not do anything
